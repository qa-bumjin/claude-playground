...
- chunk 분석 결과는 가능하면 `tc_inventory.json`에 누적 반영한다
- 이 문서는 **Claude의 작업 규칙**이다. 아래 절차는 자동 보장이 아니라, 사용자가 첨부한 파일과 현재 세션 문맥을 기준으로 수행한다
...
기획서 입력
  ...
  → 기능 인벤토리 생성 또는 기존 tc_inventory.json 보완
  → rule 정의
  → TC 생성 (기능별 순차, 문서 규모에 따라 세션당 처리 기능 수 조정)
  → 생성 직후 자체 점검: rule 누락 / boundary 누락 / ASSUMED 확인
  → CSV 저장
  → 필요 시 Excel 마스터 반영
  → tc_inventory.json의 tc_status 갱신
  → tc_progress.json은 집계용으로만 갱신
```

### 운영 현실화 원칙

- `tc_inventory.json`이 **상태의 단일 기준(source of truth)** 이다
- `tc_progress.json`은 요약/집계용이다. 진행 판단은 `tc_inventory.json`의 `tc_status`를 우선한다
- 자동 재개, 자동 diff 비교, 자동 폴더 선택을 **전제하지 않는다**
- 이어서 작업할 때는 가능하면 `tc_inventory.json`, `tc_progress.json`, 기존 CSV를 함께 첨부한다
- 세션 종료 전에는 Claude의 자체 점검과 별도로 로컬 검증 스크립트 실행을 권장한다

## 🚫 의미 중복 금지 원칙 (필수)

같은 현상을 다른 feature/파일에 반복해서 쓰지 않는다.
기획서 목차가 분리되어 있어도, 아래 4개가 사실상 같으면 **중복 TC**로 간주한다.

- 사전 조건이 같다
- 테스트 단계가 같다
- 기대 결과가 같다
- 검증 의도만 표현을 바꿔 다시 쓴 수준이다

### feature 역할 분리 기준

- `정책/운영 규칙` feature:
  최대 개수, 우선순위, 승격, 최신순 적용, 만료 처리 같은 **판정 규칙**을 검증한다
- `유저 화면/노출` feature:
  화면 위치, 노출 영역, 레이아웃 차이, 진입 화면 차이 같은 **화면별 표현 차이**를 검증한다
- `등록/수정` feature:
  입력, 저장, 유효성 검사, 수정 가능 범위 같은 **관리 기능 동작**을 검증한다

### 중복 판단 예시

- `이미지 아이콘 최대 2개 노출`은 규칙 검증이므로 `정책`에만 둔다
- `2단 배열/상세/스와이프에서 우측 상단 노출`은 화면 차이 검증이므로 `유저 화면`에 둔다
- `정책`에서 이미 검증한 최대 개수/최신순/승격 규칙을 `유저 화면`에서 동일 조건으로 다시 쓰지 않는다

### 작성 절차에 반드시 포함할 점검

TC를 CSV로 저장하기 전 아래 순서로 스스로 점검한다.

1. 각 feature의 역할을 먼저 한 줄로 정의한다
2. 새로 쓰는 TC가 기존 CSV의 어느 TC와 다른지 설명할 수 없는 경우 중복으로 본다
3. 규칙 검증과 화면 검증이 섞여 있으면 하나만 남기고 다른 하나는 위치/화면 차이만 남겨 재작성한다
4. 기존 CSV와 동일한 사전 조건/단계/기대 결과 조합이 있으면 새 TC를 만들지 않는다

---

## 🗂️ 파일 저장 위치

기획서마다 독립 폴더를 `output/tc_ready/` 하위에 생성한다. PDF 파일명에서 버전 기준으로 폴더명을 만든다.

```
프로젝트 루트/
  ├── input/                              ← 원본 기획서 PDF만 보관
  │     ├── 기획서_v1.0.3.pdf
  │     └── 기획서_v1.0.5.pdf
  └── output/
        ├── tc_ready/                     ← 1단계: TC 작성 완료, 테스트 시작 전 (에이전트 출력)
        │     └── [문서번호_문서명_버전]/  ← PDF 제목 기반 폴더 (예: CDP-20260204C_아이콘관리_v1.0.5)
        │           ├── tc_inventory.json ← 기능 인벤토리
        │           ├── tc_progress.json  ← 세션 진행 상태
        │           ├── tc_output/        ← TC CSV 파일들
        │           │     ├── TC_ICN001_기능명.csv
        │           │     └── ...
        │           └── TC_마스터.xlsx    ← Excel 마스터 파일
        ├── active_project/               ← 2단계: 현재 테스트 진행 중 (수동 이동)
        │     └── [문서번호_문서명_버전]/
        └── completed/                    ← 3단계: 테스트 완료 (수동 이동)
              └── [문서번호_문서명_버전]/
```

### 폴더명 생성 규칙

- PDF 파일명의 공백을 `_`로 치환하고 날짜 부분은 제거
- 예: `CDP-20260204C 아이콘 관리 v1.0.5 20260407.pdf` → `CDP-20260204C_아이콘관리_v1.0.5`
- 버전 업데이트 시 새 버전 폴더를 `tc_ready/` 에 신규 생성 (이전 버전 폴더 유지)

### 프로젝트 전환 규칙

- **TC 작성 완료 → 테스트 시작**: `output/tc_ready/[폴더명]/` → `output/active_project/[폴더명]/` 으로 수동 이동
- **테스트 완료**: `output/active_project/[폴더명]/` → `output/completed/[폴더명]/` 으로 수동 이동
- **새 TC 작성 시**: `output/tc_ready/` 하위에 새 폴더 생성 후 작업
...
- **Excel 마스터**: `output/tc_ready/[문서폴더]/TC_마스터.xlsx`
- **CSV 출력**: `output/tc_ready/[문서폴더]/tc_output/TC_*.csv`
...
# ── 기획서별로 변경하는 부분 ──────────────────────────────
DOC_FOLDER  = "output/tc_ready/CDP-20260204C_아이콘관리_v1.0.5"  # PDF명 기반 폴더
CSV_FILES   = [f"{DOC_FOLDER}/tc_output/TC_XXX001_기능명.csv", ...]  # 포함할 CSV 목록
OUTPUT_FILE = f"{DOC_FOLDER}/TC_마스터.xlsx"
SHEET_NAME  = "시트명"
# ─────────────────────────────────────────────────────────
...
### 세션당 처리 기능 수 기준 (문서 규모 기반)

| 기획서 규모 | 예상 기능 수 | 세션당 처리 기능 수 |
|------------|------------|-----------------|
| ~50페이지 | ~15개 이하 | **절반(7~8개)까지** 한 번에 처리 가능 |
| 51~100페이지 | ~30개 | **3~5개** |
| 100페이지 초과 | 30개 이상 | **1~2개** |

> 단, 한 기능당 rule이 15개 이상이거나 TC가 30개를 넘을 것으로 예상되면 그 기능에서 세션을 나눈다.
...
```json
{
  ...
  "generated_files": [
    "tc_output/TC_PAY001_결제기본흐름.csv",
    "tc_output/TC_PAY002_결제예외처리.csv"
  ],
  "last_updated_feature": "F-005",
  "notes": [
    "집계 기준 파일",
...
파일 첨부 없이 짧게 말할 수도 있지만, **정확성을 위해 상태 파일 첨부를 권장**한다.

| 상황 | 입력 예시 |
|------|----------|
| 이어서 작성 | `"이어서 해줘"` |
| 특정 기능부터 | `"F-007부터 해줘"` |
| 버전 업데이트 후 이어서 | `"v1.0.5로 바뀐 부분 중심으로 이어서 해줘"` |

권장 동작:
1. 사용자가 첨부한 `tc_inventory.json`을 읽어 `tc_status` 기준으로 이어서 처리할 기능을 정한다
2. `tc_progress.json`은 채번과 작업 히스토리 확인용으로만 참고한다
3. 기존 CSV를 읽어 중복 생성을 피한다
4. 첨부가 없거나 상태가 불명확하면, Claude는 자동 추정 대신 먼저 현재 기준 파일을 확인 요청한다
...
1. 구버전 PDF, 신버전 PDF, 기존 `tc_inventory.json`을 함께 첨부하는 것을 권장한다
2. Claude는 변경된 기능/섹션을 식별하려 시도하되, **정밀 diff를 자동 보장하지 않는다**
3. 변경 영향이 명확한 기능의 TC만 우선 재검토/재생성한다
4. 삭제 또는 무효화가 확인된 TC는 `Comment` 또는 별도 메모에 무효 사유를 남긴다
5. 변경 영향이 불명확하면 `ASSUMED` 또는 `needs_review`로 남기고 사람 검토를 요청한다
...
PDF 파일은 **반드시 텍스트 추출 방식**으로 읽는다. 이미지 변환 방식은 사용하지 않는다.

- **1순위 (권장)**: `pdfminer.six` — 한국어 CID 폰트 인코딩 처리가 가장 정확함
  ```python
  from pdfminer.high_level import extract_text
  text = extract_text('파일.pdf')
  ```
- **2순위**: `PyMuPDF(fitz)` — 빠르지만 비표준 폰트 인코딩 시 글자 깨짐 발생 가능
- **사용 금지**: `pdftotext -enc UTF-8` — 출력 인코딩만 바꾸며 폰트 매핑 문제 해결 안 됨
- **사용 금지**: 이미지 렌더링 후 OCR 처리
...
모든 TC 생성 이후 **반드시 검증**을 수행한다. 단, Claude의 자체 점검만으로 완료 처리하지 말고 로컬 검증 결과를 함께 본다.

### Coverage 목표

- 기본 목표: **모든 rule 최소 1개 이상 TC 연결**
- 경계값 대상 rule: **BV-MIN / BV-EXACT / BV-MAX 3종 존재**
- `ASSUMED=Y` TC는 별도 검토 대상으로 분리
...
> 로컬 검증 스크립트 예시: `python3 scripts/validate_tc_outputs.py --project output/active_project/[문서폴더]`

---

## ⛔ 세션 관리 규칙

- 세션당 처리 기능 수는 **문서 규모 기반 기준**을 따른다 (TC 생성 실행 전략 섹션 참고)
- 중단 시점까지 완료된 결과만 정리하여 출력한다
- 진행 중이던 기능은 `tc_inventory.json`에 `tc_status: in_progress` 또는 `needs_review`로 표시한다
- 이미 생성된 TC는 **손실 없이 유지**해야 한다
- 기능 처리 완료 후 CSV 저장 전 반드시 **해당 기능의 rule 수 vs 생성된 TC 수를 대조**하여 미커버 rule이 없는지 확인한다
- 세션 종료 전 가능하면 로컬 검증 스크립트를 실행한다
...
### 다음 세션 시작 방법
tc_progress.json 과 tc_inventory.json, 관련 CSV를 첨부하고 아래와 같이 입력하세요:
...
| 기대 결과 문장에 `~이고`, `~이며`, `~되고`, 쉼표로 복수 결과 연결 | 기대 결과가 2개임을 나타내는 신호 — 즉시 TC 분리 |
...
2. ✅ **Rule 기준 누락 없음** — 모든 rule이 최소 1개 이상 TC로 변환됨
...
```
(v1.2.pdf, v1.3.pdf, tc_inventory.json 동시 첨부)
변경된 기능 후보를 먼저 식별하고, 영향 있는 TC만 업데이트해줘.
```
